...
const ViewerEditScreen = ({ history, match, userInfo, viewerCreated }) => {
  ...
  const fetchEditViewer = async () => {
    try {
      ...
      const { data } = await axios.put(
        `/api/viewers/edit/${viewer._id}`,
        { name, rating, followingSince, personalityType, location, age, notes },
        config
      );
      setViewer(data);
    } catch (error) {
  ...
  return (
    <Fragment>
      {viewerCreated ? (
        <Button
          className="btn-danger btn my-5"
          onClick={(e) => removeSubmitHandler(e)}
        >
          Cancel
        </Button>
      ) : (
      ...
      {fetchEditViewerError && (
        <Message variant="danger">{fetchEditViewerError}</Message>
      )}
      {fetchViewerError && (
        <Message variant="danger">{fetchViewerError}</Message>
      )}
      <FormContainer>
        ...
        <Form onSubmit={submitHandler}>
          ...
          <Form.Group controlId="personalityType">
            ...
            <Link
              ...
              delay={200}
              isDynamic={true}
              onClick={() => setPersonalityDescription(true)}
              className="view-desc ml-3 scroll-link"
            >
              View descriptions
            </Link>
            <Form.Control
