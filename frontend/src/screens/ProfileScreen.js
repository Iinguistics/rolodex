...
const ProfileScreen = ({
  ...
  const [twitchGeneralDataLoading, setTwitchGeneralDataLoading] =
    useState(true);
  ...
  const fetchViewers = async (keyword = "", pageNumber = "") => {
    try {
      ...
      const { data } = await axios.get(
        `/api/viewers?keyword=${keyword}&pageNumber=${pageNumber}`,
        config
      );
  ...
  const fetchLiveTwitchData = async () => {
    if (userInfo) {
      const { data } = await axios.post("/api/twitchdata/livedata", {
        name: userInfo.name,
      });
  ...
  const fetchGeneralTwitchData = async () => {
    try {
      if (userInfo) {
        const { data } = await axios.post("/api/twitchdata/generaldata", {
          name: userInfo.name,
        });
      ...
      setGeneralTwitchDataError(
        "To use all of spybooks features please sign in with a valid twitch account"
      );
      setTwitchGeneralDataLoading(false);
    }
  };
  ...
  const createViewerSnapshot = async () => {
    try {
      ...
      await axios.post(
        "/api/snapshot",
        { count: liveTwitchData.viewer_count, title: liveTwitchData.title },
        config
      );
    ...
    if (listViewers) {
      return listViewers.map((viewer) => {
        return (
          <ViewerCard
            viewer={viewer}
            viewerCreatedHandlerReset={viewerCreatedHandlerReset}
          />
        );
      });
    }
  ...
  const renderGeneralTwitchData = () => {
    if (generalTwitchData) {
      return (
        <>
          <Image
            src={generalTwitchData.profile_image_url}
            roundedCircle
            fluid
            className="border border-primary shadow-lgg mb-2"
          />
          <h5 className="cap-type">
            Twitch {generalTwitchData.broadcaster_type}
          </h5>
  ...
  return (
    <div className="my-5 container">
      <div>
        <h1>Dashboard</h1>
        <div className="mb-4 user-settings">
      ...
      {createViewerError && (
        <Message variant="danger">{createViewerError}</Message>
      )}
      {listViewersError && (
        <Message variant="danger">{listViewersError}</Message>
      )}
      {createSnapshotError && (
        <Message variant="danger">{createSnapshotError}</Message>
      )}
      {captureDataError && (
        <Message variant="danger">{captureDataError}</Message>
      )}
      {generalTwitchDataError && (
        <Message variant="danger">{generalTwitchDataError}</Message>
      )}
      ...
      <Row className="mt-5">
        <Col className="mb-5" sm={6}>
          <Card
            ...
            className="profile-description-cards"
          >
            <Card.Body>
              <Card.Title className="text-white">
                Save current viewer count
              </Card.Title>
              ...
              <Card.Text>
                {liveTwitchData ? (
                  <Card.Text>
                    Click save to capture your current viewer count. You can
                    view all your captures charted out by clicking the view
                    captures.
                  </Card.Text>
                ) : (
                  <>
                    <Card.Text>
                      You must be live to capture your current viewers
                    </Card.Text>
                    <Card.Link
                      type="submit"
                      className="btn-primary btn my-3 disabled"
                    >
                      Save
                    </Card.Link>
                  </>
              ...
              {captureData.length !== 0 && (
                <Link
                  to="/profile/viewer/captures"
                  className={
                    liveTwitchData
                      ? `btn-primary btn ml-3`
                      : `btn-primary btn d-inline`
                  }
                >
                  View Captures
                </Link>
        ...
        <Col sm={6}>
          ...
          >
            <Card.Body>
              <Card.Title className="text-white">
                Add Viewer's to your book
              </Card.Title>
              <Card.Subtitle className="mb-2 text-muted">
                <VscOpenPreview />
              </Card.Subtitle>
              <Card.Text>
                You can reference back to your book. This tool allows you to
                learn more about your viewer's and grow your channel.
              </Card.Text>
              <Card.Link
                type="submit"
                className="btn-primary btn my-3"
                onClick={createViewerHandler}
              >
                + Viewer
              </Card.Link>
            </Card.Body>
          ...
          {totalAddedViewers > 0 ? (
            <>
              ...
              {totalAddedViewers > 1 && (
                <input
                  type="submit"
                  className="btn-info btn my-3"
                  value={
                    toggleSort ? "Unsort viewers" : "Sort Viewers by name (a-z)"
                  }
                  onClick={() => sortToggle()}
                />
              )}
              <h5 className="mt-4">
                {totalAddedViewers} Viewer's currently in your book <FaBook />
              </h5>
            </>
          ) : (
            <h5 className="mt-4">
              You have no viewers currently in your book. <FaBook />
            </h5>
          )}
        </Col>
      </Row>
